#!/bin/bash
# secure_deploy.sh - Deployment hardening for AgentSutra
# Run this after transferring to a dedicated runtime user or production environment.
set -e

AGENTSUTRA_DIR="${1:-$(cd "$(dirname "$0")/.." && pwd)}"
BACKUP_DIR="${AGENTSUTRA_DIR}/backups"

echo "AgentSutra Secure Deploy"
echo "======================"
echo "Target: ${AGENTSUTRA_DIR}"
echo ""
...
CRON_CMD="0 3 * * * tar czf ${BACKUP_DIR}/workspace_\$(date +\\%Y\\%m\\%d).tar.gz -C ${AGENTSUTRA_DIR} workspace/ storage/ projects.yaml 2>/dev/null; find ${BACKUP_DIR} -name 'workspace_*.tar.gz' -mtime +7 -delete"

# Check if cron job already exists
if crontab -l 2>/dev/null | grep -q "AgentSutra workspace backup"; then
    echo "  Cron job already installed, skipping."
else
    # Add new cron job preserving existing entries
    (crontab -l 2>/dev/null; echo "# AgentSutra workspace backup (daily 3am, keep 7 days)"; echo "$CRON_CMD") | crontab -
...
echo "  chmod 644 ${AGENTSUTRA_DIR}/projects.yaml"
echo "  chmod 644 ${AGENTSUTRA_DIR}/.env"
